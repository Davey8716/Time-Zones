...
class UiSmokeTests(unittest.TestCase):
    ...
    def test_window_builds_all_rows_and_refreshes(self):
        ...
        try:
            ...
            self.assertEqual(subtitle.text(), "Live UTC offsets · daylight-saving aware")
            self.assertEqual(subtitle.alignment(), Qt.AlignmentFlag.AlignCenter)
            self.assertIn("QLabel#appSubtitle", APP_STYLE)
            self.assertIn("color: #ffffff", APP_STYLE)
            self.assertIn("font-size: 8.5pt", APP_STYLE)
            self.assertIn("font-weight: 700", APP_STYLE)
    ...
    def test_country_search_centres_and_highlights_matching_offset(self):
        ...
        try:
            ...
            self.assertIn("QLabel#countrySearchLabel", APP_STYLE)
            self.assertIn("color: #ffffff", APP_STYLE)
            self.assertIn("font-size: 8.5pt", APP_STYLE)
            self.assertIn("font-weight: 700", APP_STYLE)
            window.show()
